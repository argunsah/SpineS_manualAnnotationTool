function choise=bttnChoiseDialog(dlgOptions, dlgTitle, defOption, qStr, bttnsOrdering)
%% bttnChoiseDialog
...
if isempty (bttnsOrdering)
   bttnsOrdering=zeros(1, 2);
   bttnsOrdering(1)=ceil( sqrt(nButtons) );
   bttnsOrdering(2)=ceil( nButtons/bttnsOrdering(1) );   
end

if bttnsOrdering(1)>1 && bttnsOrdering(1)<=nButtons
   bttnRows=bttnsOrdering(1);
   bttnCols=ceil( nButtons/bttnRows );
else
   if bttnsOrdering(2)>1 && bttnsOrdering(2)<=nButtons
      bttnCols=bttnsOrdering(2);
   else
